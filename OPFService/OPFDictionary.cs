...
namespace OPFService
{
    class OPFDictionary
    {
        ...
        public Boolean contains(string word)
        {
            ...
            var validationNumberUppercaseLowercase = new Regex(@"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)");

            if (!validationNumberUppercaseLowercase.IsMatch(word))
            {

                using (EventLog eventLog = new EventLog("Application"))
                {
                    eventLog.Source = "Application";
                    eventLog.WriteEntry("Password attempt not contains: Number, Uppercase letters, Lowercase letters" + word, EventLogEntryType.Information, 101, 1);
                }
                return true;
            }         
